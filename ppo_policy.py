...
from baseline_policy import FeasibilityBaseline
...
class SB3PPOPolicy:
    def __init__(self, env: BerghainEnv, policy_path: Optional[str] = None,
        ...
        if policy_path is not None and os.path.exists(policy_path):
            ...
            self.model = PPO.load(policy_path, env=self.monitor_env)   # <--- no **ppo_kwargs
        else:
    ...
    def collect_teacher_dataset(self, teacher, episodes: int = 300, keep_only_success: bool = True, seed: int = 42):
        """
        Roll out the teacher on a fresh single env (same scenario) and collect (obs, action).
        """
        e = BerghainEnv(self.env.scenario, seed=seed)
        teacher_env = e  # bind teacher to the same env instance
        if hasattr(teacher, 'env'):
            teacher.env = teacher_env  # ensure teacher reads the right counts/accepted/etc.

        X_attrs, X_acc, X_rej, X_rem, Y = [], [], [], [], []

        for ep in range(episodes):
            obs, info = e.reset(seed=seed + ep)
            done = False
            ep_buf = []
            while not done:
                a = teacher.act(obs)
                # store obs as seen by the network (Floatify shapes)
                x = self._prepare_single_obs(obs)
                ep_buf.append((x, a))
                obs, reward, term, trunc, info = e.step(a)
                done = term or trunc

            success = bool(info.get('success', False))
            if success or not keep_only_success:
                for x, a in ep_buf:
                    X_attrs.append(x['attrs'])
                    X_acc.append(x['accepted'])
                    X_rej.append(x['rejected'])
                    X_rem.append(x['remaining'])
                    Y.append(a)

        dataset = {
            'attrs': np.asarray(X_attrs, dtype=np.int8),
            'accepted': np.asarray(X_acc, dtype=np.float32),
            'rejected': np.asarray(X_rej, dtype=np.float32),
            'remaining': np.asarray(X_rem, dtype=np.float32),
            'actions': np.asarray(Y, dtype=np.int64)
        }
        return dataset

    def pretrain_with_teacher(self, teacher=None, episodes: int = 300, epochs: int = 5, batch_size: int = 2048, seed: int = 42):
        """
        Supervised BC on teacher actions before PPO.
        """
        if teacher is None:
            teacher = FeasibilityBaseline(BerghainEnv(self.env.scenario, seed=seed))

        data = self.collect_teacher_dataset(teacher, episodes=episodes, keep_only_success=True, seed=seed)
        N = data['actions'].shape[0]
        if N == 0:
            print("BC pretrain skipped: empty dataset (no successful teacher episodes).")
            return

        indices = np.arange(N)
        optim = self.model.policy.optimizer
        ...
        for epoch in range(epochs):
            ...
            for start in range(0, N, batch_size):
                ...
                batch = {
                    'attrs': data['attrs'][idx],
                    'accepted': data['accepted'][idx],
                    'rejected': data['rejected'][idx],
                    'remaining': data['remaining'][idx],
                }
                # normalize with the current VecNormalize stats (no stats update here)
                batch_norm = self.monitor_env.normalize_obs(batch)

                obs_tensor, _ = self.model.policy.obs_to_tensor(batch_norm)
                acts = torch.as_tensor(data['actions'][idx], device=device)
    ...
    def evaluate(self, eval_episodes: int = 10) -> tuple[float, float]:
        ...
        eval_vec = VecNormalize(
            ...
            norm_reward=True,
            clip_obs=10.0,
            norm_obs_keys=["accepted", "rejected", "remaining"],  # <-- add this
        )
...
from ppo_policy import SB3PPOPolicy
